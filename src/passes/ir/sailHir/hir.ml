...
struct
  ...
  let lower_expression (e : AstParser.expression) : expression ECSW.t = 
    ...
    match e with 
      | Variable id -> 
        ...
        ECSW.throw_if_none Error.(make info @@ Fmt.str "undeclared variable '%s'" id)  v >> fun () ->
        return {info; exp=Variable id}
      | Deref e -> 
  ...
  let lower_process (p:p_in process_defn) (env,decls:HIREnv.t) sm : (p_out * HIREnv.D.t) E.t = 
    ...
    let sv = List.map fst p.p_shared_vars |> FieldSet.of_list  in 
    let locals = List.map fst p.p_body.decls |> FieldSet.of_list  in 

    let has_name_conflict = FieldSet.(union params sv |> union locals |> cardinal <> cardinal params + cardinal sv + cardinal locals) in 
    E.throw_if Error.(make dummy_pos @@ Fmt.str "process '%s' : name conflict between params,local decls or shared variables" p.p_name) has_name_conflict >> fun () ->

    let add_vars v e = ListM.fold_left (fun e (id,_) -> HIREnv.declare_var id (dummy_pos,()) e) (e,decls) v >>| fst in 
    let* env = add_vars p.p_body.decls env >>= add_vars p.p_shared_vars in


    let* init,decls =  lower_method (snd p.p_body.init,()) (env,decls) sm |> E.recover ({info=dummy_pos;stmt=Skip},decls) in
    let+ decls,loop = ListM.fold_left_map (fun (decls : D.t) -> function 
                            | l,Statement s -> let* x,e = lower_method (s,()) (env,decls) sm in return (e,(l,Statement x))
                            | l,Run (proc,args,sv) -> 
                              let open MonadFunctions(ECSW) in
                              let* (args,_),decls = ListM.map lower_expression args (env,decls) |> ECS.run  in 
                              return (decls,(l,Run (proc,args,sv)))
